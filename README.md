...
- **Streaming replies with Stop** — type a question and press `Enter`; press `Enter` again (or click Stop) to cancel a running reply. Assistant replies render **compact Markdown** (headings, lists, code, tables) that stays tight inside the chat area — the panel layout and input are unchanged, and long replies scroll inside the message box instead of pushing the Send button away.
- **Model & context chip** — with an item selected, the context bar shows `model · used / limit tok · %` so you can see how much of the model context window the conversation uses (limit configurable in Settings).
