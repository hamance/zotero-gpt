...
### Stage 5e — UI/visualization pass: scroll island, Markdown, bigger UI, model/context chip (2026-09-09)
- **Ask (user):** (1) chat should feel like its own dock — wheel-scrolling inside the chat currently jumps the item pane; (2) render Markdown; (3) bigger input/text areas; (4) show LLM info such as context length.
- **Dock constraint (verified in Zotero 10 `omni.ja`):** the only supported docking API is `Zotero.ItemPaneManager.registerSection` (no `Sidebar.registerSidebar`, no `Reader.registerReaderTabPanel`), so a truly separate window/dock is not available. Instead the panel is now a **scroll island**: `.zoterogpt-messages` has its own `overflow-y:auto` + `overscroll-behavior:contain` (min 300px / max 60vh), and a passive `wheel` listener on the panel calls `stopPropagation()` so the item pane never scrolls while interacting with the chat.
- **Markdown (`src/modules/markdown.ts`, new):** `markdown-it` with `html:false` + a `DOMParser` sanitizer (strips `on*` handlers and `javascript:`/`data:` URLs); assistant bubbles render into `.markdown-body` and the bundled `addon/content/md.css` is linked; user bubbles stay plain text. `punycode` added as a dependency (required by markdown-it for the browser bundle).
- **Bigger UI:** panel 14px, messages min 300px, textarea 3 rows (min-height 64px), larger buttons/send/padding, focus ring on the input.
- **LLM info:** a status chip in the context bar shows `model · <usage> / <contextLimit> tok · <pct>%`, updated on every message sync. Token estimate = latin chars/4 + CJK chars (1). New pref `contextLimit` (default 128000, editable in settings).
- Tests: new `test/markdown.test.ts` (render/sanitize/escape/token estimate), panel tests for the status chip + `contextLimit` persistence.
